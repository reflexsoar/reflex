...
if [ -z "$install" ]; then
    ...
    sudo --preserve-env=FLASK_CONFIG --preserve-env=PIPENV_PIPFILE -u reflex -i PWD=/opt/reflex/reflex-api pipenv run python manage.py db init
    echo "Performing any necessary migrations"
    sudo --preserve-env=FLASK_CONFIG --preserve-env=PIPENV_PIPFILE -u reflex -i PWD=/opt/reflex/reflex-api pipenv run python manage.py db migrate
    echo "Running db upgrades"
    sudo --preserve-env=FLASK_CONFIG --preserve-env=PIPENV_PIPFILE -u reflex -i PWD=/opt/reflex/reflex-api pipenv run python manage.py db upgrade
    echo "Installing reflex API"
    sudo --preserve-env=FLASK_CONFIG --preserve-env=PIPENV_PIPFILE -u reflex -i PWD=/opt/reflex/reflex-api pipenv run python manage.py setup
fi
